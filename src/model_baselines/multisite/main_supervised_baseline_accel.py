# encoding=utf-8
"""
main_supervised_baseline_accel.py
---------------------------------
Multisite training with green + accel_z modality (8-channel input).

  * --single_device None  : 8 channels (4 devices × green+accel_z)
  * --single_device 0-3   : 2 channels (one device's green+accel_z)
                            0=earring, 1=ring, 2=watch, 3=necklace

Usage:
    # 4-device fusion (8 channels)
    python main_supervised_baseline_accel.py --backbone resnet --target_domain P1

    # Single device + accel_z (2 channels)
    python main_supervised_baseline_accel.py --backbone resnet --target_domain P1 --single_device 0

    # All participants (LOSO sweep)
    python main_supervised_baseline_accel.py --backbone resnet

Note: Run `python align.py --modality accel --data_dir <path>` first to
generate the aligned_8channel_accel.npz files.
"""
...
parser = argparse.ArgumentParser(
    description='Supervised baseline (green + accel_z) — PPG HR regression'
)
...
def train_sup(args, seed_idx=0):
    ...
    sd_tag = (f"_{DEVICE_NAMES[args.single_device]}Accel"
              if args.single_device is not None else "_8chAccel")
    args.model_name = (
        f"{args.backbone}_multisite_accel{sd_tag}"
...
if __name__ == '__main__':
    ...
    if not participants:
        raise FileNotFoundError(
            f"No aligned_8channel_accel.npz found in '{args.data_dir}'. "
            f"Run `python align.py --modality accel --data_dir <path>` first."
        )

    # Single participant run (debug / test)
    ...
    # Full LOSO sweep over all participants
    else:
